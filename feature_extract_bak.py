...
class LSTM(Extractor):
    ...
    def ext_forward(self, observations: Dict):
        ...
        for item in observations:
            ...
            #    out = out.reshape((batch_len, -1))
                data_list.append(out)
            elif item in ["graph", "look_back_pct"] :
                # res = {item, observations[item]}
                g_dlist[item] = observations[item]
            #    out = ou"it.reshape((batch_len, -1))
            #    data_list.append(out)
            else:
                pass
            #    data_list.append(observations[item])
        # 
        #g_d = [g_dlist["look_back_pct"], g_dlist["graph"]]
        #g_res = []
        #batch_len = len(g_d[0])
        #for i in range(batch_len):
        #    tmp_res, _ = self.ext_feat["graph"]([g_d[0][i], g_d[1][i][0]])
        #    g_res.append(tmp_res)
        #m = [item.reshape(-1) for item in g_res]
        #m = torch.stack(m)
        # g_t = self.ext_feat["graph"](g_d)
    ...
    def module_dict(self) -> Dict:
        ...
        for item in self._observation_space.spaces:
            ...
            #                              kernel_size=self.cfg["cnn_kernel_size"])
                ...
                nn_dict["graph"] = GCN(input_dim=self.cfg["g_input_size"],hidden=self.cfg["gcn_hidden"],output_dim=self.cfg["gcn_out_size"])
            #    nn_dict[item] = nn.Conv2d(in_channels=1, out_channels=self.cfg["cnn_out_channels"],
            #                              kernel_size=self.cfg["cnn_kernel_size"])
            else:
                pass
