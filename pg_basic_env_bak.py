...
class PGBasicEnv(gym.Env):
    ...
    def __init__(self, cfg: Dict,tic_list: List) -> None:
        ...
        spaces={
                ...
                "look_back_pct": Spaces.Box(-1, 1, shape=(self.cfg["asset_num"], self.cfg["look_back"]))
        }
    ...
    def get_simi(self):
        ...
        for i in range(5):
            ...
            if  np.linalg.norm(Z-Zold,ord='fro') < 0.0000001:
                break
        return Z
    ...
    def _step(self, action: np.ndarray) -> tuple:
        ...
        # sell to total money
        self.total_money = (stock_share * out_pxs).sum()-0.0001 * (last_stock_money * np.maximum(self.wlist[-1]-action,0)).sum()
        # 0.14% 的交易成本
